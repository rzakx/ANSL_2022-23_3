...
app.post("/listaUzytkownikow/", (req, res) => {
	db.query("SELECT `konta`.`id` as 'kontoid', `konta`.`login`, `konta`.`typkonta`, `konta`.`kiedydolaczyl`, `konta`.`awatar`, `konta`.`stawka`, `konta`.`garaz`, `konta`.`truck`, `konta`.`discord`, `konta`.`steam`, `konta`.`truckersmp`, `konta`.`truckbook`, `typkonta`.`nazwa` FROM `konta`, `typkonta` WHERE `typkonta`.`id` = `konta`.`typkonta` ORDER BY `konta`.`typkonta` ASC",
	(err, result) => {
...
app.post("/miasta", (req, res) => {
	db.query("SELECT `id`, `miasto`, `kraj`,`gra` FROM `miejscowosci` ORDER BY `id` ASC", (er, result) => {
		if(result.length > 0){
			let tmp = [];
...
app.post("/podwyzkaOdrzuc/:token", (req, res) => {
	console.log(req.params.token, "odrzucil wniosek o ID:", req.body.idwniosku);
...
app.post("/podwyzkaAkcept/:token", (req, res) => {
	console.log(req.params.token, " zaakceptowal wniosek o ID: ", req.body.idwniosku);
	db.query("UPDATE `podwyzka` SET `wniosek` = 1, `wniosektxt` = 'Zaakceptowano', `ktorozpatrzyl` = (SELECT `id` FROM `konta` WHERE `token` = ?) WHERE `id` = ?", [req.params.token, req.body.idwniosku], (err, r) => {
		if(r.affectedRows > 0){
			db.query("UPDATE `konta` SET `stawka` = ?, `typkonta` = ? WHERE `id` = ?", [req.body.stawka, req.body.typkonta, req.body.idwnioskujacego], (er2, r2) => {
...
app.post("/administrujProfil/:token", (req, res) => {
	console.log(req.params.token, " zaktualizowal profil o ID: ", req.body.idosoby);
	db.query("UPDATE `konta` SET `login` = ?, `stawka` = ?, `garaz` = ?, `truck` = ?, `typkonta` = ?, `kiedydolaczyl` = ?, `steam` = ?, `discord` = ?, `truckersmp` = ?, `truckbook` = ? WHERE `id` = ?",
	[req.body.login, req.body.stawka, req.body.garaz, req.body.truck, req.body.typkonta, req.body.datadolaczenia, req.body.steam, req.body.discord, req.body.truckersmp, req.body.truckbook, req.body.idosoby], (err, r) => {
		if(err) console.log(err);
		if(r.affectedRows > 0) {
...
app.post("/usunKonto/:token", (req, res) => {
	console.log("UWAGA!!!", req.params.token, "USUNĄŁ KONTO O ID:", req.body.idosoby);
	//winiety
					...
					db.query("DELETE FROM `dysphistoria` WHERE `kto` = ?", [req.body.idosoby], (er5, r5) => {
						...
						db.query("DELETE FROM `dodawaniekwoty` WHERE `komu` = ?", [req.body.idosoby], (er6, r6) => {
							...
							db.query("DELETE FROM `trasy` WHERE `kto` = ?", [req.body.idosoby], (er7, r7) => {
								//uprawnienia
								db.query("DELETE FROM `uprawnienia` WHERE `kto` = ?", [req.body.idosoby], (er8, r8) => {
									//konto
...
app.post("/twojeAktDaneWniosek/:token", (req, res) => {
	if(req.params.token){
		db.query("SELECT `id`,`typkonta`,`stawka` FROM `konta` WHERE `token` = ?", [req.params.token], (err, r) => {
			if(r.length > 0){
				res.send({idosoby: r[0]['id'], aktstawka: r[0]['stawka'], aktstanowisko: r[0]['typkonta']});
			} else {
...
//ustaw limit km
app.post("/ustawLimit", (req, res) => {
	db.query("UPDATE `ustawienia` SET `wartosc` = ? WHERE `nazwa` = 'limit_km'", [req.body.limit], (err, r) => {
		if(r.affectedRows > 0){
			res.send({odp: 'OK'});
		} else {
...
app.post("/ustawPowitalna", (req, res) => {
	db.query("UPDATE `ustawienia` SET `wartosc` = ? WHERE `nazwa` = 'informacja'", [req.body.tresc], (err, r) => {
		if(r.affectedRows > 0){
			res.send({odp: 'OK'});
		} else {
			res.send({blad: 'Nie ustawiono'});
...
app.post("/dodajKwote/:token", (req, res) => {
	db.query("INSERT INTO `dodawaniekwoty` (`komu`, `kwota`, `kto`, `powod`) VALUES (?, ?, (SELECT `id` FROM `konta` WHERE `token` = ?), 'Brak powodu')", [req.body.komu, req.body.kwota, req.params.token], (err, r) => {
		if(err) console.log(err);
		if(r.affectedRows > 0){
			res.send({odp: 'OK'});
		} else {
